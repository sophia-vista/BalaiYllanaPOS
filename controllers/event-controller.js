...
const eventController = {
    ...
    postCancelPencilBooking: function (req, res) {
        db.updateOne(Event, { _id: req.query.id }, {status: 'cancelled'}, function(event){
            res.render('event', { layout: false, data: event }, function (err, html) {
                res.send(html);
            });
        })
    },
    ...
    postCreatePencilBooking: function (req, res) {
        ...
        db.insertOne(Event, event, function (result) {
            if (result)
                res.redirect('/event-tracker/pencilbookings');
        });
    },
    ...
    getCalendar: function (req, res) {
        ...
        let calendarArray = calendarObj.monthDays(currentDate.getFullYear(), currentDate.getMonth());
        let eventArray = [];
            ...
            for (let j = 0; j < 7; j++) {
                ...
                if (calendarArray[i][j] === 0) {
                    ...
                    eventItem = {
                        ...
                        afternoon: ((Math.floor(Math.random() * 2) === 0) ? true : false),
                        evening: ((Math.floor(Math.random() * 2) === 0) ? true : false)
                    };
                }
